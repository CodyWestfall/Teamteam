...
<!-- NavBar -->
		...
		<div class=" col-md-2">
			<div class="text-center" id="buttons">
				...
				<form action="./upload.php" method="POST" enctype="multipart/form-data">
					<input class="" type="file" style="width: 150px" name="image" id="image"/>
					<br />
					<br />
					<input class="btn btn-default" style="width: 150px" type="submit" value="Upload" />
				</form>
			</div>
		</div>
    ...
    <script>
		...
		function deleteNode() {
			var serialId, serialNo, mdl, mdlContent, message, buttonContainer, noButton, yesButton;

			serialId = event.target.id;
			serialNo = document.getElementById(serialId).getAttribute('title');
            ...
            message = $('<div class="message"></div>');
            message.text('Are you sure you want to delete this node?');

            buttonContainer = $('<div class="buttonContainer"></div>');
            mdlContent.append(message, buttonContainer);

            noButton = $('<button>No</button>');

            yesButton = $('<button>Yes</button>');
